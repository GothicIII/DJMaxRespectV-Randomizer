...
## FIRST RUN
  ...
  When running the 1st time some info messages will appear and the program creates files/folder to run properly.
  ...
  more sorting options onto the table but it is really hard to keep track of the sorting functions.
...
## USING THE RANDOMIZER
  ...
  toggles all (enabled) song-packsn at once. Untoggled song-packs are not rolled but the order of each song is preserved.
  ...
  a right-mouse-button menu where you can toggle each Pack individually. Toggling only a few from a category will mark the
  corresponding checkbox with a '-'.
  ...
  K-Modes, difficulties and the star-difficulty slider - you know what they do. Each option is checked against the current
  available song table and if a combination is impossible, those difficulties will be grayed out. If no song can be rolled
  due to e.g. most beeing excluded, you will be informed.
...
## EXCLUDING CHARTS
  ...
  This will be written to DJMaxExcludeCharts.db and the chart won't be rolled again (default setting). You have about 3 seconds to
  undo the change before it gets written by pressing 'F4' or clicking the checkbox.
  ...
  In the Options (general tab) you can even force to roll only charts when the previous difficulty is excluded. So when you start
  from a fresh file you'll only get NM charts. Excluding them will unlock the HD-chart of that song and so on. Very useful for practicing.
